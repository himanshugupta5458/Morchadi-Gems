...
| Build fails: `Cannot find module '@/scripts/keyword-normalisation.mjs'`, but `npm run build` is green locally | `.dockerignore` excludes a path the build actually imports | Not an import bug — the file never reached the build context. `.dockerignore` excludes `scripts`, and `lib/keyword-collision-check.ts` imports one file out of it, so that file carries an explicit `!scripts/keyword-normalisation.mjs` negation. Any new non-test `@/scripts/*` import needs its own negation line. A local build cannot catch this — `.dockerignore` applies only when a build context is assembled. See [this log](docs/logs/2026-08-28-docker-build-cannot-find-module-scripts-keyword-normalisation.md). |
