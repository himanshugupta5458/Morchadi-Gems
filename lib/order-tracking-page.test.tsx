 ...
 */
const LEAKABLE_VALUES: Readonly<Record<string, string>> = {
...
function leakableValue(label: keyof typeof LEAKABLE_VALUES): string {
  return LEAKABLE_VALUES[label];
}
...
describe("the fixture order, as the database actually holds it", () => {
  it("carries every value the page is then asked not to show", async (ctx) => {
    ...
    for (const [label, value] of Object.entries(LEAKABLE_VALUES)) {
...
describe("what /track renders for a real order", () => {
  ...
  it("puts none of the twenty private values anywhere in the HTML", async (ctx) => {
    ctx.skip(unavailableReason !== null, unavailableReason ?? undefined);
