...
| TC-02 | Catalogue size | Count entries | At least one; the exact count is reported, not asserted — see [ADR-020](../decisions/ADR-020-two-tier-catalogue-ia.md). 49 as of [ADR-021](../decisions/ADR-021-all-real-catalogue.md) | Automated |
| TC-03 | Identifier uniqueness | Collect all `id` values into a set | Set size equals entry count; no duplicates | Automated |
| TC-04 | Identifier convention | Match each `id` against `^P\d{3}$` | **Every** id is a P-code. The placeholder `nk-001` form is no longer accepted, which is what keeps invented products out of the catalogue — see [ADR-021](../decisions/ADR-021-all-real-catalogue.md) | Automated |
| TC-05 | Category validity | Check each `category` against the ten known slugs | No unknown slug | Automated |
| TC-06 | Category coverage | Count products per category | All ten categories hold at least one product. Restored as a hard failure now that `watches` and `hair-accessories` are stocked | Automated |
...
| TC-09 | Price range | Bucket prices into budget / mid / premium | Zero products fall outside ₹25–₹25,000. Floor lowered from ₹100 for P047 at ₹49 | Automated |
| TC-10 | Images shape | Check `images` is an array of non-empty strings | Array in every case, even with one placeholder | Automated |
| TC-11 | Details shape | Check `details.material` required; `weight`, `closure`, `type`, `stone`, `size` optional but non-empty when present | Conforms to `ProductDetails`. Weight is optional for every product now that every product is the owner's own | Automated |
...
| TC-20 | Out-of-stock coverage | Count `inStock === false` across the catalogue | At least one, so the sold-out UI keeps coverage. Six as of ADR-021 | Automated |
...
| TC-23 | Discount ceiling | Compute `(mrp - price) / mrp` per product | At most 80% on every product — the placeholder-only 60% ceiling is gone with the placeholders. See [ADR-003](../decisions/ADR-003-discount-display-pricing.md) | Automated |
...
| TC-26 | Owner's photography on disk | Check `public/products/P0NN.webp` exists for each P-code | All 49 present; covered by TC-10's on-disk check, called out here because these are irreplaceable files rather than regenerable placeholders | Automated |
...
| TC-N2 | Misspell a category as `neckalces` | TC-05 fails. TC-04 no longer interacts with the category — the id check is a plain P-code match — so the slug is reported once |
| TC-N3 | Set a `rating` to `7.4` | TC-13 fails |
| TC-N4 | Set a `price` to `"1200"` (string) | TC-08 fails |
| TC-N5 | Drop `featured` below four | TC-18 fails with the observed count |
| TC-N6 | Add a product with the id `nk-001` | TC-04 fails — the placeholder id form is no longer a valid id, which is the check that keeps the catalogue all-real |
